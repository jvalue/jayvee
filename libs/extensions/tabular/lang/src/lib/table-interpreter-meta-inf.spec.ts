...
describe('Validation of TableInterpreterMetaInformation', () => {
  ...
  it('should diagnose error on non unique column names', async () => {
    const text = readJvTestAsset(
      'table-interpreter-meta-inf/invalid-non-unique-column-names.jv',
    );
    ...
    expect(diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          message: 'The column name "name" needs to be unique.',
        }),
        expect.objectContaining({
          message: 'The column name "name" needs to be unique.',
        }),
