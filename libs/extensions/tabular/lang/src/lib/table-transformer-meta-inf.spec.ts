...
describe('Validation of TableTransformerMetaInformation', () => {
  ...
  it('should diagnose error on number of input columns do not match transform input ports', async () => {
    const text = readJvTestAsset(
      'table-transformer-meta-inf/invalid-input-columns-transform-port-missmatch.jv',
    );
    ...
    expect(diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          message: 'Expected 1 columns but only got 2',
        }),
      ]),
    );
  });
